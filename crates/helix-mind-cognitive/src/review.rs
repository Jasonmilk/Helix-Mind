...
/// 检视输出相对旧路径实体覆盖度的增量阈值（≥ 视为僵化）。
const STALE_ENTITY_DELTA: usize = 2;
...
impl SleepReview {
    ...
    ///
    /// 检视增量 ≥ STALE_ENTITY_DELTA → 旧路径僵化（Stale）；否则仍有效（Viable）。
    pub fn review_path(&self, legacy_entity_count: usize, review_entity_count: usize) -> ReviewVerdict {
        if review_entity_count.saturating_sub(legacy_entity_count) >= STALE_ENTITY_DELTA {
            ReviewVerdict::Stale
        } else {
...
mod tests {
    ...
    fn stale_when_review_finds_much_more() {
        ...
        assert_eq!(SleepReview.review_path(1, 4), ReviewVerdict::Stale);
        // 恰好在阈值（差 2）→ 僵化。
        assert_eq!(SleepReview.review_path(1, 3), ReviewVerdict::Stale);
    }

    #[test]
    fn viable_when_review_finds_similar_or_less() {
        assert_eq!(SleepReview.review_path(4, 4), ReviewVerdict::Viable);
        assert_eq!(SleepReview.review_path(4, 5), ReviewVerdict::Viable, "差 1 不僵化");
        assert_eq!(SleepReview.review_path(5, 2), ReviewVerdict::Viable, "更少则旧路径更优");
    }
}
