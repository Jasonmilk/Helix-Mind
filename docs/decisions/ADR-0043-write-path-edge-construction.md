...
**理由**：节点写入是 upsert（`sqlite_pool.rs` 的 `ON CONFLICT ... DO UPDATE`），而 `edges` 表在 `add_edge` 上是**纯追加**。同一次写入被重放（Anaphase 重试、`op` 重放）会建出重复边，重复边会让行归一化把同一份能量分给同一条关系两次，**静默改变扩散权重**。故建边前须先判「该 (source,target,relation) 是否已存在」。
...
### D8：写入必须保持**无环**（INTENT-7 的硬要求，且机制已存在）

INTENT-7 §3.2 的原文是「将新沉淀的结论或观测结果**追加写入记忆有向无环图**」——**DAG 是协议要求**，不是本仓的偏好。因此写入路径在按 `parent_ids` 建边时**必须拒绝会成环的父引用**，而不是建出一条环边。
**复用**：`MemoryTopology::would_create_cycle(source, target)` **已经存在**（`topology.rs`），无需新机制。
**理由**：边一旦成环，行归一化下的幂迭代仍在 `ρ(αW) ≤ α < 1` 下收敛，所以**不会报错、不会发散**——它会静默地把能量在两个节点间来回泵送，使两个节点都获得虚高的激活。这是「不报错的错」，必须靠显式拒绝而非靠事后观测。
...
1. **D7 的 `Temporal` vs 沿用 `Refines`**：前者语义更准但引入第二套约定；后者一致但把「续接」说成「精炼」。
