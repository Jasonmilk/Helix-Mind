...
**线上验收（2026-09-17，T5a+T5b 落地并重启实测）**：

| 指标 | 周期前 | 第 1 个周期后 | 第 2 个周期后 |
|---|---|---|---|
| `nodes` | 545 | 548 | 550 |
| `edges` | **0** | **40** | **60** |
| 边的不同来源数 | — | 2 | 3 |
...
新边全部是 `Temporal` / `weight 0.8` / **方向派生者 → 来源**，且新节点的 `derived_from` 与边的目标集**逐一吻合** —— 整条链（Anaphase 周期 → 检索 → `memory_nodes` → `parent_ids` → gRPC → Mind 的 `derived_from` + 边）**线上贯通**。触发方式：经面板 `/api/chat` 走真实 LLM 周期。

**一个必须说清的结构事实**：第 1 个周期后是**星形而非链**（2 个来源 / 40 个目标 / **0 个中间节点**）⇒ 实际深度只有 **1 跳**。跨周期才会成链，第 2 个周期出现 **1 个中间节点** ⇒ **2 跳路径成立**，与「下一周期的父集含上一周期的新节点」的预测一致。⇒ **ADR-0042 D0 恢复的多跳能力已可达，但线上目前只到 2 跳**；深度随周期累积增长，不应据此宣称「多跳扩散已验证充分」。

**一处观察（不作因果断言）**：第 1 个周期回答「我不知道你最喜欢的数字」，第 2 个周期回答 **「7」**（正确）。变化与上一周期写入的新节点落入检索集同时发生，但本 ADR 不据此断言因果。
