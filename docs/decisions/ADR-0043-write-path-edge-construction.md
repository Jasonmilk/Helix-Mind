...
2. 平行边让同一条关系在 `sum_abs` 与 `a_next` 里**各计两次**，**静默改变扩散权重**（这正是 ADR-0042 D0 那类「数值静默走偏」的同一族病）。
...
| 场景 | relation_type | weight | 方向 | 理由 |
|---|---|---|---|---|
| L2 抽象自 L3（crystallize 既有） | `Refines` | 0.8 | 派生者 → 来源 | **既有约定，不动** |
| L3 情节续接（第 N 轮承接第 N−1 轮） | `Temporal` | 0.8 | 派生者 → 来源 | 「时间上接续」是 `Temporal` 的定义；用 `Refines` 描述会把它说成「精炼」，语义不实 |
| 调用方显式声明的其它关系 | 由请求携带（**本期不做**） | — | — | 需要一个 `relation` 字段，属独立决策；本期只处理 `derived_from` 这一种语义 |

映射表**必须落在一处**（Mind 侧写入路径），不得在 Anaphase 侧重复定义。
**⚠️ 这是本 ADR 最需要人类确认的一条**：`edges` 是 Append-Only，关系类型选错**难以事后纠正**。

### D8：写入必须保持**无环**——机制**已存在**，写入路径只需**容忍拒绝**

INTENT-7 §3.2 的原文是「将新沉淀的结论或观测结果**追加写入记忆有向无环图**」——**DAG 是协议要求**。

**实测：这条已经被强制执行了。** `StorageEngine::add_edge` 对**硬边**先做 `would_create_cycle`，成环则返回 `MindError::CycleDetected`（`engine.rs:194-203`；软边跳过检查）。因此本决策**不需要新机制**，只需要规定**写入路径如何应对这个拒绝**：

> **节点写入与边建立必须可分离**：某个父引用若会成环，应**跳过那条边并记录**，而不是让整个 `Remember` 请求失败。节点本身是合法的、且已被写入；因为一个父引用成环而回滚整次记忆写入，会用「拓扑约束」去否决「记忆事实」。

**为什么值得显式写下来**：边一旦成环**不会报错、不会发散**（`ρ(αW) ≤ α < 1` 仍收敛），它会静默地在两节点间来回泵送能量、抬高两者激活。这是「不报错的错」，只能靠显式拒绝。
...
1. ~~**D7 的 `Temporal` vs 沿用 `Refines`**~~ → **已由执行者决定，取 `Temporal`**（2026-09-17，依人类「剩下的优先级你决定」的授权）。理由：人类同时裁定「现在是测试，原始数据不重要」⇒ 关系类型选错可重建，「Append-Only 难以纠正」这条顾虑随之失效；于是取**语义诚实**的那一个，而不是为统一约定把「续接」说成「精炼」。**若人类有异议，这是最容易改的一处**（改一个映射表条目 + 重建测试库）。
2. ~~`parent_ids` 是否就是 `derived_from` 的同一物~~ → **已由原文证据解决，不再是分歧**：INTENT-7 §3.2 `WRITE_NODE` 的 `params` 是 `{node_id, type, parent_ids, content}`，且该节说明是「追加写入记忆**有向无环图**」⇒ `parent_ids` = **DAG 父节点** = `Node.derived_from` 的语义。本 ADR 据此把它当作同一物处理（并因此引出 D8 的成环拒绝，见上）。
