...
├── Cellrix/                          ← 展示层（空间语义终端 UI）—— **皮肤**；其 WebUI 的设计源是 `lumtract/`（见下）
├── Tuck/                             ← 安全闸门（免疫系统）
...
├── lumtract/                         ← **Lumtact・水之波光**设计体系的可执行载体（推导器）—— Cellrix 的 WebUI 设计源，见 §0.1
...
### 0.1 Lumtact・水之波光（设计哲学）与 Cellrix 的关系

**Lumtact 是本生态的设计哲学层，Cellrix 是它的实现皮肤。** 两者此前只在 §8 的
2026-09-09 变更日志行里被提到（"WebUI 按「水之波光 · 触境」重做"），导航性段落里查不到 ——
本节把它补到该在的位置。
...
> Lumtract 是 Lumtact 的可执行载体：推导器不产生理念，它让理念成为可验证、可审计的产物。
> 水没有光，是虚空；光没有水，是无处落脚的射线。人（用户与设计者）与之相遇，才有波光。

**核心主张**：每一个设计值都可追溯到 —— 一条经过验证的**目的**、一组被诚实的**约束**、
一次清晰的**冲突裁决**、一个明确的**来源标注**。界面里的 `[PHYS:*]` / `[GENE]` / `[PURPOSE]`
标注即这套溯源编号；`Cellrix:ADR-0015` 引用它们（如 `[PHYS:P-016]`）即因此。

**三道关卡**（查三件不同的事，谁都替代不了谁）：

| 关卡 | 命令 | 查什么 |
|---|---|---|
| ① 约束校验 | `node scripts/lumtract-verify.mjs` | 是否违反钻石约束（123 项：钻石/钢铁 114 + 工程阈值 9） |
| ② 契约测试 | `npx vitest run` | 令牌契约、层级单调、零硬编码、导出一致性 |
| ③ 类型检查 | `npm run build` | 类型是否闭合（**唯一不可替代**：①管不了类型，②只擦除类型不校验） |

**硬度三级**（约束本身也分硬度，且**校验规则自己也要有来源标注**）：
...
**两条铁律与八项禁止**（对本生态的其它仓同样有约束力）：

- 「**把陶土说成钻石，是推导中最常见的欺诈**」—— 把手段冒充约束，再用自动化固化它，
  等于把这个错误变成不可违抗的律令。
- 禁止推导方式：类比推导（跨通道混淆）／**权威推导（从表象倒推）**／经验推导（无法追溯约束）／
  流行推导（流行不是约束）／审美惯性（品牌冒充目的）／**基因冒充约束（`[GENE]` 冒充 `[PHYS]`）**／
  目的豁免／模糊量化。

> **两条直接适用于本生态的工程实践**：① 拿"某个成熟项目这么做"当设计理由，是**权威推导**；
> ② 拿项目血统/身份当硬约束，是**基因冒充约束**。二者都必须换成可追溯到目的与物理事实的理由。
>
> **固化时机**：Lumtract **刻意不配 pre-commit hook** —— 固化的前提是约束已被充分质疑，
> 而当前所有 `[PURPOSE]` 仍为临时值。本生态的 `helix-mind/tools/hooks`（大改动护栏 + ADR 首行校验）
> 与此不冲突：它守的是"文件被整份覆盖"这一**事故形状**，不是设计值的正确性。
